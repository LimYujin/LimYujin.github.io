...
<p> 
&nbsp;&nbsp;&nbsp;&nbsp;주요 이슈
- 봇이 작동하지 않거나, 디스코드 자체와 충돌하는 현상이 있음
- 파이썬 3.5.x이상의 버전에서 작동 오류가 날 때가 있음
&nbsp;&nbsp;&nbsp;&nbsp; 기여 방안
- 디스코드 버전이 업그레이드 되었을 때 호환성을 꾸준히 유지하게 하는 것
- 한글로도 사용이 가능하게끔 하는 것
</p>
...
나는 번역엔 약해서 git 저장소 관리와 소스코드 추가를 담당하였다. 자세한 내용은 아래 정리와 같다.
...
이슈를 여러 개로 분류하여 팀원들이 파악하기 쉽게 하고, 팀 repository의 소통을 위해서도 이슈를 활용하며, wiki라는 것을 이용해 팀 관련 소개와 활동 내역들을 보여주고, 프로젝트를 계획 중인 프로젝트, 진행 중인 프로젝트, 작업이 끝난 프로젝트 등으로 나누어서 정리를 깔끔하게 하고 있었다. 내가 아직 큰 소스코드 관련 기여는 잘 못하더라도, git 저장소 관리 역할을 맡음으로써 팀프로젝트에 기여하고 싶다.
